...
public class MapRenderer extends Canvas {
  ...
  public MapRenderer(Map map) {
    ...
    characterSprite.addAnimationState(
    ...
    targetSprite.addAnimationState(
      Character.State.IDLE,
      "src/main/resources/sprites/target.png"
    );

    treeSprite = new AnimatedSprite(128, 128);
    treeSprite.addAnimationState(
      Character.State.IDLE,
  ...
  private void drawTarget(GraphicsContext gc) {
    targetSprite.draw(
      gc,
      Constants.getWorldCoordinates(map.getTargetCell().getIndex()),
      Character.State.IDLE
    );
  }
  ...
  private void drawCharacter(GraphicsContext gc) {
    characterSprite.draw(
      gc,
      map.getCharacter().getPos(),
      map.getCharacter().getMoveDirection()
    );
  }
  ...
  private void drawPath(List<Cell> path, Color color) {
    ...
    for (int i = 0; i < path.size(); i++) {
      ...
      Point center = Constants.getImageCoordinates(
        Constants.getWorldCoordinates(cell1.getIndex())
      );
      xPoints[i] = center.x;
      yPoints[i] = center.y;
  ...
  private void drawCell(
    ...
    point[0] =
      Constants.getImageCoordinates(
        new Point(
          (int) (center.x - 0.5 * Constants.CELL_RENDER_SIDELENGTH),
    ...
    point[2] =
      Constants.getImageCoordinates(
        new Point(
          (int) (center.x + Constants.CELL_RENDER_SIDELENGTH),
          (int) (center.y)
        )
      );
    point[3] =
    ...
    // cell area
    Color cellSmellColor = new Color(
      0.75 - cell.getSmell() / maxSmell * 0.5,
      0.75 - cell.getSmell() / maxSmell * 0.5,
      0.2,
      1
    );
  ...
  private void drawCell(GraphicsContext gc, Cell zelle, float maxSmell) {
    ...
    Point mittelpunkt = Constants.getWorldCoordinates(zelle.getIndex());
    drawCell(gc, zelle, maxSmell, mittelpunkt);

    if (zelle.isOccupied()) {
      treeSprite.draw(
        gc,
        Constants.getWorldCoordinates(zelle.getIndex()),
        Character.State.IDLE
      );
    }
  }
